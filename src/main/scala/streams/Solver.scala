...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    def olo(neighbours: List[(Block, Move)]): Stream[(Block, List[Move])] = {
      if (neighbours.isEmpty) Stream.empty
      else neighbours match {
        case (block, move) :: xs => (block, move :: history) #:: olo(xs)
      }
    }

    olo(b.legalNeighbors)
  }
   ...
   */
  lazy val pathsFromStart: Stream[(Block, List[Move])] = from(neighborsWithHistory(startBlock, List()), Set())
    //from((startBlock, List[Move]()) #:: Stream[(Block, List[Move])](), Set())
